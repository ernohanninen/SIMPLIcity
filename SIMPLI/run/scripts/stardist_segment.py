...
def parse_tiff_metadata(file_name, sample):
	...
	with open(file_name) as metadata_file:
		...
		if "sample_name" not in reader.fieldnames or not set(all_labels).issubset(reader.fieldnames):
			print('"sample_name" or one or more image labels missing in the channel metadata header:')
...
def extract_features(cell_label_image, tiff_image_metadata, sample, counter, marker):
	...
	"""
	"""if exists(cell_label_image):
		print("____________________________________________________________-")
		file = open(cell_table_filename)
		reader  =csv.reader(file)
		objects = len(list(reader))
		file.close()
	else: objects = 0"""
...
if __name__ == "__main__":
	...
	if model.config.n_channel_in == 1:
		...
		if len(image_metadata) == 2:
			...
			images.append(numpy.zeros_like(images[1]))
			#images.append(numpy.zeros_like(images[1]))   
		...
		if len(image_metadata) >= 3: #TEST THIS
			print("OK")
