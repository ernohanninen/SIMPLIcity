...
process normalize_tiffs {
    ...
    label 'big_memory'
    process.container = 'ernohanninen/r_container'
...
process operetta_image_preprocessing{
    ...
    script:
    //Outputs images, metadata for all images from all samples and by sample metadata for the preprocessed images (in long and cp compatible format)
    //I think that the preprocessed_tiff_metadata file is not created in this scipt
    """
	python3 $projectDir/scripts/operetta_image_preprocessing.py \\
        $sample_name \\
        $tiff_input_metadata_file \\
        ./ \\
...
process image_preprocessing {
    ...
    //I think that the preprocessed_tiff_metadata file is not created in this scipt
    """
	python3 $projectDir/scripts/image_preprocessing.py \\
        $sample_name \\
        $normalized_metadata \\
        ./ \\
...
process sd_cell_segmentation {
    ...
    script:
    """
	python3 $projectDir/scripts/stardist_segment.py \\
		$sample_name \\
...
process cell_intensity_visualization{
    ...
    """
	python3 $projectDir/scripts/Cell_Intensity_plotter.py \\
        $preprocessed_tiff \\
        $cell_overlay_tiff \\
        $sample_metadata_file \\
...
process compute_cell_area {
    ...
    script:
    """
	python3 $projectDir/scripts/Cell_area_measurements.py \\
        $cell_overlay_tiff\\
        $annotated_cell_data_file \\
        $cell_masking_metadata \\
        $cell_types
