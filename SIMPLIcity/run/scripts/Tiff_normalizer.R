#Source: https://github.com/ciccalab/SIMPLI/blob/master/scripts/Tiff_normalizer.R
...
process_image_single <- function(image, sample_name, marker, label,thresholding, output_dir)
{
  ...
  return(c(sample_name = sample_name, marker = marker, label = label, thresholding = thresholding, file_name = output_file_name))
}
...
if (file_type == "ome") {
  ...
  metadata <- lapply(seq(1, raw_metadata[, .N]), function(n){
    ...
    process_image_single(my_image, meta$sample_name, meta$marker, meta$label, meta$thresholding, output_path)  
  })
...
metadata_columns <- metadata_columns[!(metadata_columns %in% c("sample_name", "label", "marker", "thresholding"))]
