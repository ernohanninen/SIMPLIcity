#Source: https://github.com/ciccalab/SIMPLI/blob/master/scripts/Cell_Threshold_Plotter.R
...
library(uwot)
library(EBImage)
source("/opt/scripts/Plot_Functions.R")
...
if(length(sample_names) > 1){
  for (pop in populations){
    multi_pdf_plotter(list(sample_barplots[[pop]], comparison_barplots[[pop]]),
      filenamePdf = paste0(barplot_output_folder, "/", pop, "-barplots.pdf"), filenamePng = paste0(barplot_output_folder, "/", pop, "-barplots.png"), n_col = 1, n_row = 2)
  }
...
if(n_categories == 2){
  ...
  for (pop in populations){
    multi_pdf_plotter(comparison_boxplots[[pop]], filenamePdf = paste0(boxplot_output_folder, "/",
      pop, "-boxplots.pdf"), filenamePng = paste0(boxplot_output_folder, "/",
      pop, "-boxplots.png"))
  }
}
