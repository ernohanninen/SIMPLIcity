...
#Source: https://github.com/ciccalab/SIMPLI/blob/master/scripts/Area_measurer.R
...
get_ROI_areas <- function(samples)
{
  ROI_areas <- sapply(samples, function(name){
    img <- tiff_loader(image_metadata[sample_name == name, file_name2][[1]])
    dim(img)[[1]] * dim(img)[[2]]
  })
...
process_image <- function(name_sample, expressions)
{
  ...
  marker_file_names <- sapply(image_names, function(marker_name){
    image_metadata[sample_name == name_sample & label == marker_name, file_name2]})
  cat("\n\n")
